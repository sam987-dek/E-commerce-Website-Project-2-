...
const Products = () => {
  ...
  return (
    <div className="page-container min-h-[80vh]">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-5 mb-8 border-b-2 border-slate-200 pb-6">
        <div>
          <h1 className="text-4xl font-black text-slate-900 tracking-tight capitalize">{urlCategory ? urlCategory.split('-').join(' ') : 'All Products'}</h1>
          <p className="text-slate-500 font-medium mt-2 flex items-center gap-2">
            Showing <span className="text-slate-900 font-bold bg-slate-100 px-2 py-0.5 rounded shadow-inner">{filteredProducts.length}</span> result(s)
            {urlSearch && <span>for <strong className="text-blue-600 ml-1">"{urlSearch}"</strong></span>}
          </p>
        </div>
        
        <div className="flex items-center gap-3 w-full md:w-auto">
          <button 
            className="md:hidden flex items-center justify-center gap-2 bg-slate-900 text-white px-5 py-3 rounded-xl font-bold hover:bg-slate-800 transition-colors shadow-md w-full sm:w-auto"
          ...
          <div className="relative w-full md:w-80 hidden sm:block">
            ...
            <input 
              ...
              className="w-full bg-white border-2 border-slate-200 rounded-xl pl-12 pr-4 py-3 font-semibold text-slate-700 focus:outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 transition-all shadow-sm"
            />
          </div>
        </div>
      </div>
